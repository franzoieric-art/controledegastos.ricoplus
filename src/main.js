...
const App = {
    ...
    bindGlobalEventListeners() {
        ...
        document.body.addEventListener('click', (event) => {
            ...
            if (t.closest('.ai-analysis-btn')) App.ai.getFinancialAnalysis(parseInt(t.closest('.ai-analysis-btn').dataset.monthIndex));
            
            // CORREÇÃO E ESTILIZAÇÃO DO BOTÃO ANUAL
            if (t.matches('#ai-annual-analysis-btn') || t.closest('#ai-annual-analysis-btn')) {
                console.log("Botão IA Anual Clicado"); // Debug visual
                App.ai.getAnnualFinancialAnalysis();
            }
            ...
            if (t.closest('.suggest-category-btn')) {
                ...
                if (el) {
                    ...
                    if (descriptionInput && descriptionInput.value) {
                        App.ai.getCategorySuggestion(descriptionInput.value, entryId, btn);
                    }
                }
            }
    ...
    ai: {
        async getFinancialAnalysis(monthIndex) {
            ...
            try {
                ...
                App.ui.aiAnalysisResult.innerHTML = `<div class="prose dark:prose-invert max-w-none">${cleanedResponse}</div>`;
            } catch (error) {
    ...
    render: {
        ...
        createEntryElement: (config) => {
            ...
            let r = '', p = '', c = '', s = '', aiBtn = '';
            if (type === 'expense') {
                r = `<button class="remove-btn w-8 h-8 rounded-full bg-transparent hover:bg-red-100 text-gray-400 hover:text-red-500 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100" data-type="expense" data-month-index="${monthIndex}" data-day="${dayIndex}" data-category="${category}" data-entry-id="${entry.id}"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M8.75 1A2.75 2.75 0 006 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 10.23 1.482l.149-.022.841 10.518A2.75 2.75 0 007.596 19h4.807a2.75 2.75 0 002.742-2.53l.841-10.52.149.023a.75.75 0 00.23-1.482A41.03 41.03 0 0014 4.193V3.75A2.75 2.75 0 0011.25 1h-2.5zM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4zM8.58 7.72a.75.75 0 00-1.5.06l.3 7.5a.75.75 0 101.5-.06l-.3-7.5zm4.34.06a.75.75 0 10-1.5-.06l-.3 7.5a.75.75 0 101.5.06l.3-7.5z" clip-rule="evenodd" /></svg></button>`;
                ...
                r = `<button class="remove-btn w-8 h-8 rounded-full bg-transparent hover:bg-red-100 text-gray-400 hover:text-red-500 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100" data-type="${type}" data-month-index="${monthIndex}" data-entry-id="${entry.id}"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M8.75 1A2.75 2.75 0 006 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 10.23 1.482l.149-.022.841 10.518A2.75 2.75 0 007.596 19h4.807a2.75 2.75 0 002.742-2.53l.841-10.52.149.023a.75.75 0 00.23-1.482A41.03 41.03 0 0014 4.193V3.75A2.75 2.75 0 0011.25 1h-2.5zM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4zM8.58 7.72a.75.75 0 00-1.5.06l.3 7.5a.75.75 0 101.5-.06l-.3-7.5zm4.34.06a.75.75 0 10-1.5-.06l-.3 7.5a.75.75 0 101.5.06l.3-7.5z" clip-rule="evenodd" /></svg></button>`;
            }
        ...
        renderCardList: () => { App.ui.cardListContainer.innerHTML = App.state.creditCards.map(c => `<div class="flex items-center justify-between p-3 rounded-xl bg-[var(--input-bg)] border border-[var(--border-color)] mb-2 group hover:border-[var(--primary-color)] transition-colors"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded-lg bg-blue-100 text-blue-600 flex items-center justify-center"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg></div><span class="font-medium text-[var(--text-color)]">${c}</span></div><button class="remove-card-btn w-8 h-8 rounded-full hover:bg-red-100 text-gray-400 hover:text-red-500 flex items-center justify-center transition-all" data-card-name="${c}"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" /></svg></button></div>`).join(''); },
        renderCategoryList: () => { App.ui.categoryListContainer.innerHTML = App.state.categories.map(c => `<div class="flex items-center justify-between p-3 rounded-xl bg-[var(--input-bg)] border border-[var(--border-color)] mb-2 group"><div class="flex-grow flex items-center gap-3 mr-3"><div class="w-8 h-8 rounded-lg bg-purple-100 text-purple-600 flex items-center justify-center flex-shrink-0"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg></div><input type="text" value="${c.name}" class="category-name-input w-full bg-transparent font-medium text-[var(--text-color)] outline-none border-b border-transparent focus:border-[var(--primary-color)] transition-colors" data-old-name="${c.name}"></div><div class="flex items-center gap-2"><div class="relative"><span class="absolute left-2 top-1/2 transform -translate-y-1/2 text-xs text-[var(--muted-text)]">R$</span><input type="number" value="${c.budget}" min="0" class="category-budget-input w-24 pl-6 pr-2 py-1.5 rounded-lg bg-[var(--card-bg)] border border-[var(--border-color)] text-sm font-semibold text-right outline-none focus:border-[var(--primary-color)] transition-colors" data-category-name="${c.name}"></div><button class="remove-category-btn w-8 h-8 rounded-full hover:bg-red-100 text-gray-400 hover:text-red-500 flex items-center justify-center transition-all" data-category-name="${c.name}"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" /></svg></button></div></div>`).join(''); },
        renderRecurringList: () => { App.ui.recurringListContainer.innerHTML = App.state.recurringEntries.map((r, i) => `<div class="p-3 rounded-xl bg-[var(--input-bg)] border border-[var(--border-color)] mb-2 flex justify-between items-center"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded-lg bg-orange-100 text-orange-600 flex items-center justify-center flex-shrink-0"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div><div><p class="font-semibold text-sm text-[var(--text-color)]">${r.description}</p><p class="text-xs text-[var(--muted-text)]">Dia ${r.dayOfMonth} • ${App.helpers.formatCurrency(r.amount)} • ${r.type}</p></div></div><button class="remove-recurring-btn w-8 h-8 rounded-full hover:bg-red-100 text-gray-400 hover:text-red-500 flex items-center justify-center transition-all" data-index="${i}"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" /></svg></button></div>`).join(''); },
